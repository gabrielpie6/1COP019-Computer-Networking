...
def server(host, port, protocol):
    ...
    while True:
        ...
        receivedPackages, receivedBytes, elapsedTime = donwload(connection, control)
        time.sleep(1)
...
def client(host, port, protocol):
    ...
    while True:
        try:
            ...
            sentPackages, bytesSent = upload(s, sc, seconds)
...
def listenToEOT(connection: socket.socket):
    ...
    while listening:
        ...
        if(data):
            ...
            break
#
def sendPackets(s: socket.socket, control: socket.socket, seconds:float):
    startTime    = time.time()
    timeElapsed  = 0.0
    sentPackages = 0
    bytesSent    = 0
...
def upload(client, control, seconds):
    sentPackages, bytesSent = sendPackets(client, control, seconds)
    return sentPackages, bytesSent
...
if __name__ == "__main__":
    ...
    if   action == "1":
        ...
        sentPackages, bytesSent, receivedPackages, receivedBytes, elapsedTime, seconds = client(host, 5001, "TCP")
